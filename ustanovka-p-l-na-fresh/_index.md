...
1. Общий модуль **ХЛ_СистемаУчета**, необходимо удалить пароль.

2. Обработка 	**ХЛ_ФормированиеАктовСчетов,** очистить процедуру ОтправитьПоЭлектроннойПочтеНаСервере().

   ![](./_index-9.png){width=526px height=108px}

3. Документ **ХЛ_ОтчетБаланс**, удалить все процедуры на форме элемента.
...
5. В модуле **ХЛ_СинхронизацияСZenmoney** вычистить методы ОбновитьТокенДоступа() и ПолучитьОбновленныеДанные()



### Подготовка P&L Консалтинг (устарела)
...
### Подготовка P&L Консалтинг (Актуальная)

1. Не обновлять модуль приложения

2. Модуль ХЛ_СистемаУчета (по сути, можно просто его не объединять)

   1. Снять пароль с модуля ХЛ_СистемаУчета

   2. Удалить функции:

      1. ОбновленияДоступны

      2. ПолучитьСписокВерсий

      3. ПолучитьСсылкуНаДокументацию

      4. ПолучитьОписаниеПоСсылке

      5. ПолучитьФайлОбновления

      6. ПолучитьФайлОбновленияПоПолномуПути

      7. ЕстьНовыеВерсии

      8. ПолучитьНавигационнуюСсылкуОбновления

   3. Из УстановитьВидимостьОбъектовПоВерсии в ветке `ИначеЕсли Форма.ИмяФормы = "ОбщаяФорма.ХЛ_ФормаНастройки" Тогда` удалить добавление страниц «Обновление» и BI

3. Форма ХЛ_РабочийСтол.ФормаПоддержка:

   1. Удалить код поиска версии расширения и поиска обновлений

   2. Из HTML удалить блок обновлений и текущей версии
...
2. Общий модуль **ХЛ_СистемаУчета**.
   ...
   1. Все процедуры удалить кроме ЛицензияПолучена(), ДанныеЛицензииДляВывода(), УстановитьВидимостьОбъектовПоВерсии() и ПроверитьКонстантыЛицензий().
...
Из формы «Поддержка» удалить нижние 2 блока
